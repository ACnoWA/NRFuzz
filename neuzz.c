...
#define MEM_BARRIER() \
    asm volatile("" ::: "memory")
/* Map size for the traced binary. 这是256K不同于afl的64K*/
...
//read函数，从fd这个文件指针指向的文件中传送_len个字节到buf指针所指的内存中。返回值为实际读取到的字节数。
...
typedef uint8_t  u8;   // unsigned char 1个字节
...
void setup_stdio_file(void) {
  ...
  char* fn = alloc_printf("%s/.cur_input", out_dir); //seeds文件夹下的隐藏文件
...
void init_forkserver(char** argv) {
  ...
  //调用pipe函数时在内核中开辟一块缓冲区（称为管道）用于通信，通过形参传出给用户程序两个文件描述符，st_pipe[0]指向且只能指向管道的读端，st_pipe[1]只能指向管道的写端。其他形参类似。
  if (pipe(st_pipe) || pipe(ctl_pipe)) perror("pipe() failed");
  
  forksrv_pid = fork(); //fork之前只有一个进程在执行这段代码，fork之后就变成了变成了两个进程在执行，这两个进程几乎完全相同，将要执行的下一条语句都是if(forksrv_pid < 0)...


  if (forksrv_pid < 0) perror("fork() failed");
//fork()被调用一次，返回两次，有三种可能的返回值：在父进程中，fork返回新创建的子进程的进程ID；在子进程中，fork返回0;若出错，fork返回一个负值。所以下面就是子进程中要执行的
  if (!forksrv_pid) {
    ...
    //execv会停止执行当前的进程，并且以第一个参数所表示的应用进程替换被停止执行的过程，进程ID没有改变。第一参数，被执行的应用进程；第二个参数传递给应用程序的参数列表，这个数组的第一个参数是应用程序名字本身，并且最后一个参数为NULL
...
static u16 count_class_lookup16[65536];  //4*65536 = 256KB?? 所以这个是那个bitmap?


//利用lookup8去初始化lookup16数组,其实也就是对应利用lookup8这个基础数组去实现相应的次数相加，将范围扩大到65536次。例如b1 = 1, b2 = 3的时候，b1左移8位就是128，就是对在128次的基础上加了。
...
static inline void classify_counts(u64* mem) {
  ...
  while (i--) {
    ...
    if (unlikely(*mem)) { //likely和unlikely不会影响分支结果，只不过是在编译是表示一种概率，likely就表示执行if块中的内容可能性大一点，因此会将该语句块预编译到前面。所以起作用主要是提升效率。
...
void gen_mutate(){
    ...
    for(int iter=0 ;iter<13; iter=iter+1){
        memcpy(out_buf1, out_buf, len); //此处的len是命令行中-l后的那个数值        
        memcpy(out_buf2, out_buf, len);        
        
        /* find mutation range for every iteration */
        int low_index = num_index[iter]; //iter的次数刚好是num_index数组长度少一位,刚好保证下一行的iter+1最后还在数组范围
        ...
        for(int step=0;step<up_step;step=step+1){
            ...
            // 对所有位置都+1之后才保存,而且只起一个临时保存的作用，因为不是以添加的方式打开文件
            write_to_testcase(out_buf1, len);    
            // run_target通知forkserver产生一个子进程执行target
...
void fuzz_lop(char * grad_file, int sock){
    ...
    while ((nread = getline(&line, &llen, stream)) != -1) {        
        ...
        if(line_cnt == retrain_interval){
            round_cnt++;
            now = count_non_255_bytes(virgin_bits); //virgin_bits是初始化值255的一个很大的数组,vrigi_bit是用来记录总的tuple信息
            edge_gain = now - old;
            old = now;
        ...
        /* parse gradient info */
        char* loc_str = strtok(line,"|"); //strtok第一次调用需设置两个参数，第一次分割的结果，返回串中第一个'|'之前的字符串
        char* sign_str = strtok(NULL,"|"); //第二次调用，第一个参数设为NULL，结果返回分割依据之后的字符串。
	...
	FILE *rc_f;
	time(&cur_time);
	int tmp_cnt = (cur_time - f_time) / 1200;
	if (tmp_cnt > t_cnt) {
	    t_cnt = tmp_cnt;
	    rc_f = fopen("log_time.txt", "a");
	    fprintf(rc_f, "%d minutes(%ld), edge coverage: %d\n", t_cnt * 20, cur_time, count_non_255_bytes(virgin_bits));
	    fclose(rc_f);
	}

        /* read seed into mem */
        int fn_fd = open(fn,O_RDONLY); //open函数执行成功则返回文件描述符，否则返回-1
        if(fn_fd == -1){
            perror("open failed");
            exit(0);
        }
        struct stat st;
        int ret = fstat(fn_fd,&st); //fstat用于将fn_fd所指的文件的文件状态（信息）复制到结构体struct stat中。
        int file_len = st.st_size;
        memset(out_buf1,0,len); //这个len是从命令行参数获取的 
        memset(out_buf2,0,len);
        memset(out_buf,0, len);
        memset(out_buf3,0, 20000);
        ck_read(fn_fd, out_buf, file_len, fn); //从fn_fd中复制file_len长度的字节到out_buf中
...
void start_fuzz(int f_len){
    ...
    time(&f_time);
    FILE *fn_log;
    fn_log = fopen("log_time.txt", "a");
    fprintf(fn_log, "time: %ld, initial seed edge coverage: %d\n", f_time, count_non_255_bytes(virgin_bits));
    fclose(fn_log);
...
void main(int argc, char*argv[]){
    ...
    switch (opt) {
      ...
      case 'l': /* file len */
         ...
         if(len > 7000)
         {
             num_index[13] = (len - 1); //num_index[13]是数组的最后一位
    ...
    //建立共享内存
    setup_shm();
    //根据count_class_lookup8的256个位置两两得到
    init_count_class16();
    setup_dirs_fds();
